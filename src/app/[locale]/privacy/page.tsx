...
export async function generateMetadata({
  ...
  const messages = await loadMessages(locale, ["privacy.seo"]);
  const t = createTranslator(messages, "privacy.seo");
  ...
  const title =
    typeof titleValue === "string"
      ? titleValue
      : "Privacy Policy | ApolloSong";
  const description =
    typeof descriptionValue === "string"
      ? descriptionValue
      : "Learn how ApolloSong collects and protects your information.";
...
export default async function PrivacyPage({
  ...
  const { locale: localeParam } = await paramsPromise;
  const locale = isLocale(localeParam) ? localeParam : defaultLocale;
  const messages = await loadMessages(locale, ["privacy.page"]);
  const t = createTranslator(messages, "privacy.page");
