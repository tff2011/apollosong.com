...
// Load env manually to avoid t3-env validation
...
// Inline email builder for streaming-vip-in-progress (blue branding pattern)
function buildStreamingVipInProgressEmail(params: {
    orderId: string;
    ...
    const { orderId, recipientName, locale, trackOrderUrl, songName, coverUrl } = params;
    ...
    const senderName = brandNames[loc];
    const senderEmail = envVars.SMTP_FROM || "contact@apollosong.com";
    const from = `"${senderName}" <${senderEmail.replace(/.*<|>/g, "")}>`;
    ...
    const footerTexts: Record<SupportedLocale, string> = {
        en: "Made with faith and love by Apollo Song.",
        pt: "Feito com fé e amor por Apollo Song.",
        es: "Hecho con fe y amor por Canción Divina.",
        fr: "Fait avec foi et amour par ChansonDivine.",
        it: "Fatto con fede e amore da CanzoneDivina.",
    };
    ...
    const preheaders: Record<SupportedLocale, string> = {
        en: `${recipientName}'s song has been sent to the distributor - coming soon to Spotify!`,
        pt: `A música de ${recipientName} foi enviada para a distribuidora - em breve no Spotify!`,
        es: `La canción de ${recipientName} ha sido enviada a la distribuidora - ¡pronto en Spotify!`,
        fr: `La chanson de ${recipientName} a été envoyée au distributeur - bientôt sur Spotify !`,
        it: `La canzone di ${recipientName} è stata inviata al distributore - presto su Spotify!`,
    };
    ...
    const songInfoHtml = (songName || coverUrl) ? `
                            ...
                            <div style="background-color: #F8FAFC; border-radius: 12px; padding: 25px; margin-bottom: 25px; text-align: center; border: 1px solid #E2E8F0;">
                                ...
                                <p style="font-size: 22px; color: #1E40AF; margin: 0; font-weight: 600; font-family: Georgia, serif;">"${songName}"</p>
                                ` : ""}
                            </div>
    ` : "";
...
<body style="margin: 0; padding: 0; background-color: #F1F5F9; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #1E293B; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%;">
        ...
        <tr>
            <td align="center" style="padding: 40px 20px;">
                ...
                <table role="presentation" width="600" border="0" cellspacing="0" cellpadding="0" style="background-color: #FFFFFF; border-radius: 12px; overflow: hidden;">
                    ...
                    <!-- Header -->
                    <tr>
                        <td align="center" style="background-color: #1E40AF; padding: 40px 0;">
                           <!-- Text Logo -->
                           <span style="font-family: serif; font-size: 32px; font-weight: bold; color: #FFFFFF !important; letter-spacing: -0.5px;">
                               ${logoText}
                           </span>
                           ${subBrandText ? `<br><span style="font-family: Arial, sans-serif; font-size: 14px; color: #93C5FD !important;">${subBrandText}</span>` : ""}
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding: 40px 50px;">
                            <h1 style="color: #1E40AF; font-size: 32px; margin: 0 0 25px; text-align: center; font-weight: 600;">${title}</h1>
                            ...
                            <!-- CTA Section -->
                            <div style="background-color: #1E293B; border-radius: 12px; padding: 30px; margin-bottom: 35px; border: 2px solid #334155;">
                                <!-- Explicit URL -->
                                ...
                                <p style="font-size: 14px; margin: 0 0 25px; text-align: center; word-break: break-all;">
                                    <a href="${trackOrderUrl}" style="color: #60A5FA; text-decoration: underline;">${trackOrderUrl}</a>
                                </p>
                    ...
                    <tr>
                        <td style="background-color: #F8FAFC; padding: 30px; text-align: center; border-top: 1px solid #E2E8F0;">
                            ...
                            <p style="font-size: 14px; color: #64748B; margin: 0;">
                                ${footerText}<br>
                                <a href="https://${websiteUrl}" style="color: #1E40AF; text-decoration: none; font-weight: 500;">${websiteUrl}</a><br>
                            ...
                            <p style="font-size: 12px; color: #94A3B8; margin: 8px 0 0;">
                                ${unsubscribeText} <a href="https://wa.me/5561995790193?text=Descadastrar" style="color: #94A3B8; text-decoration: underline;">${unsubscribeAction}</a>
                            </p>
                        </td>
                    </tr>
                </table>
...
${unsubscribeText} -> https://wa.me/5561995790193?text=Descadastrar
    `;
...
async function sendTestEmail() {
    ...
    const emailData = buildStreamingVipInProgressEmail({
        orderId: "test_streaming_vip_123",
        recipientName: "Maria",
        locale: "pt",
        trackOrderUrl: "https://www.apollosong.com/pt/track-order?email=thiagofelizola%40gmail.com",
        songName: "Amor Eterno Para Maria",
        coverUrl: "https://pub-12e00c54e36f4c8a92310ed6a7fba969.r2.dev/streaming-covers/cm5vtmxsy0001bh8tz80m23bq-cover.png",
    });

    console.log("Sending test email to:", testEmail);
    console.log("Subject:", emailData.subject);
    console.log("SMTP Host:", envVars.SMTP_HOST);
