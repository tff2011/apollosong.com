...
const includeExtras = process.argv.includes("--extras");
...
const { buildSongDeliveryEmail } = await import("../src/server/email/song-delivery");
...
const emailContent = buildSongDeliveryEmail({
    ...
    songFileUrl2: includeTwoOptions
        ...
        : undefined,
    hasCertificate: includeExtras,
    certificateToken: includeExtras ? "test-certificate-token" : null,
    hasLyrics: includeExtras,
});
...
console.log(`Sent test delivery email to ${to}`);
