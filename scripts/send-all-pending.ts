...
async function main() {
    // Find orders with songs ready but not delivered (paid up to Jan 3)
    ...
    console.log(`\n📧 Found ${pendingOrders.length} pending orders with songs ready\n`);
    ...
    for (let i = 0; i < pendingOrders.length; i += BATCH_SIZE) {
        ...
        for (const order of batch) {
            try {
                ...
                sent++;
                const paidAt = order.paymentCompletedAt?.toISOString().split("T")[0] || "N/A";
                console.log(`  ✅ ${order.recipientName.substring(0, 15).padEnd(15)} -> ${order.email} (paid: ${paidAt})`);
            } catch (error) {
                failed++;
                console.error(`  ❌ ${order.recipientName} -> ${order.email}: ${error}`);
            }
        }
